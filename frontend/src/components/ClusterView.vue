<template>
  <div class="cluster-view">
    ...
    <v-card variant="outlined" class="mb-3">
      <v-card-text class="py-2">
        <div class="d-flex flex-column gap-2">
          ...
          <!-- Fila 3: Selector de tipo de embedding -->
          <div class="d-flex align-center gap-3">
            <div class="text-caption" style="min-width: 120px">
              <strong>Tipo de embedding:</strong>
            </div>
            <v-btn-toggle
    ...
    <v-card variant="outlined" class="mt-3">
      <v-card-text class="py-2">
        <div class="text-caption font-weight-medium mb-2">
          Leyenda de Nodos (Áreas Legales):
        </div>
        <div class="d-flex flex-wrap gap-3 mb-3">
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{ background: legalAreaColors['Penal'] }"
            ></div>
          ...
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{ background: legalAreaColors['Laboral'] }"
            ></div>
          ...
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{ background: legalAreaColors['Civil'] }"
            ></div>
          ...
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{ background: legalAreaColors['Familia Civil'] }"
            ></div>
            <span class="text-caption">Familia Civil</span>
          </div>
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{ background: legalAreaColors['Familia Tutelar'] }"
            ></div>
            <span class="text-caption">Familia Tutelar</span>
          </div>
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{
                background: legalAreaColors['Contencioso Administrativo'],
              }"
            ></div>
            <span class="text-caption">Contencioso Adm.</span>
          </div>
          <div class="d-flex align-center gap-1">
            <div
              class="legend-circle"
              :style="{ background: legalAreaColors['Sin área'] }"
            ></div>
    ...
    <!-- Temas de Clusters (Keywords) -->
    <v-card
      variant="outlined"
      class="mt-3"
      v-if="clusterStats && clusterStats.length > 0"
    >
      <v-card-title class="text-subtitle-1 d-flex align-center">
        <v-icon start>mdi-tag-multiple</v-icon>
        Temas por Cluster
        <v-chip size="x-small" class="ml-2" color="primary"
          >{{ clusterStats.length }} clusters</v-chip
        >
      </v-card-title>
      <v-card-text class="pt-0">
        <v-row dense>
          <v-col
            v-for="cluster in clusterStats"
            :key="cluster.cluster_id || cluster.cluster_label"
            cols="12"
            sm="6"
            md="4"
          >
            <v-card
              variant="tonal"
              :color="
                getClusterColor(cluster.cluster_id || cluster.cluster_label)
              "
              class="cluster-topic-card"
              density="compact"
            >
              <v-card-text class="py-2 px-3">
                <div class="d-flex align-center mb-1">
                  ...
                  <v-chip size="x-small" variant="outlined">
                    {{ cluster.dominant_area || cluster.main_area || "Mixto" }}
                  </v-chip>
                </div>

                <!-- Keywords -->
                <div
                  class="cluster-keywords mt-2"
                  v-if="cluster.keywords && cluster.keywords.length > 0"
                >
                  <v-chip
                    v-for="(kw, idx) in cluster.keywords.slice(0, 5)"
                    :key="idx"
                    size="x-small"
                    variant="outlined"
                    :color="idx < 2 ? 'primary' : 'grey'"
                    class="mr-1 mb-1"
                    :title="`Score: ${kw.score}`"
                  >
                    {{ kw.word }}
                  </v-chip>
                </div>
                <div v-else class="text-caption text-grey">
                  {{ cluster.topic_label || "Sin temas identificados" }}
                </div>
              </v-card-text>
            </v-card>
          </v-col>
...
// Composables
const { legalAreaColors } = useColors();
...
const clusterStats = ref([]); // Estadísticas de clusters con keywords
const selectedNode = ref(null);
const selectedNodeData = ref(null);
const centralDocument = ref(null);
const minSimilarityFilter = ref(0.3); // Filtro de similitud mínima
const embeddingField = ref("clean_embedding"); // Tipo de embedding para clustering
...
// Colores para clusters (distintos de áreas legales)
const clusterColors = [
  "#2196F3", // Azul
  "#4CAF50", // Verde
  "#FF9800", // Naranja
...
// Función para obtener color por cluster
const getClusterColor = (clusterId) => {
  if (clusterId === -1) return "#9E9E9E"; // Ruido en gris
...
const loadCluster = async () => {
  ...
  console.log("Using embedding field:", embeddingField.value);
  ...
  try {
    ...
    console.log("Cluster stats:", clusterStats.value);
...
// Recargar cuando cambie el tipo de embedding
watch(embeddingField, () => {
  loadCluster();
});
...
/* Tarjetas de temas de clusters */
.cluster-topic-card {
  transition: all 0.2s ease;
}
.cluster-topic-card:hover {
